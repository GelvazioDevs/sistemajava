...
public class CadastroVenda extends javax.swing.JFrame {
    ...
    private void ButtonGravarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonGravarActionPerformed
        boolean temItemNaVenda = false;
        for (int x = 0; x < tabelaItemVenda.getRowCount(); x++) {
            temItemNaVenda = true;
        }
        if(TextFieldCliente.getText().equals("")){
            JOptionPane.showMessageDialog(null, "Informe o cliente!");
        } else if(TextFieldVendedor.getText().equals("")){
           JOptionPane.showMessageDialog(null, "Informe o vendedor!");         
        } else if(TextFieldCondicaoPagamento.getText().equals("")){
           JOptionPane.showMessageDialog(null, "Informe a condição de pagamento!");       
        } else if (!temItemNaVenda){
            JOptionPane.showMessageDialog(null, "Informe pelo menos 1 item antes de gravar a venda!");
        } else {
            venda.setCodigo(Integer.parseInt(TextFieldCodigoVenda.getText()));            
            
            venda.setCliente(Integer.parseInt(TextFieldCliente.getText()));
            venda.setVendedor(Integer.parseInt(TextFieldVendedor.getText()));
            venda.setCondicaoPagamento(Integer.parseInt(TextFieldCondicaoPagamento.getText()));

            venda.setData(String.valueOf(data.getTime()));

            if (venda.gravarVenda()) {
                for (int x = 0; x < tabelaItemVenda.getRowCount(); x++) {
                    vendaProduto.setPreco(Double.parseDouble(String.valueOf(tabelaItemVenda.getModel().getValueAt(x, 3))));
                    vendaProduto.setProduto(Integer.parseInt(String.valueOf(tabelaItemVenda.getModel().getValueAt(x, 0))));
                    vendaProduto.setQuantidade(Integer.parseInt(String.valueOf(tabelaItemVenda.getModel().getValueAt(x, 2))));
    ...
    private void TextFieldCodigoItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_TextFieldCodigoItemActionPerformed
        if (!TextFieldCodigoItem.getText().equals("")) {
            ...
            if (produto.procurarProduto()) {
                ...
                TextFieldQuantidadeItem.setText("1");
    ...
    private void TextFieldValorItemKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TextFieldValorItemKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (TextFieldQuantidadeItem.getText().equals("")) {
            ...
            if (validaCampos) {
                ...
                // Adiciona a linha no grid de dados
                model.addRow(linha);
                
                // Limpa os campos de Produto que acabaram de ser utilizados
    ...
    private void ButtonDeletarItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonDeletarItemActionPerformed
        int selectedRow = tabelaItemVenda.getSelectedRow();
        if(selectedRow >= 0){
            ((DefaultTableModel) tabelaItemVenda.getModel()).removeRow(selectedRow);            
        } else {
